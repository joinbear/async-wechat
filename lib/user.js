...
class User {
	constructor(wechat,appName) {
		...
		this.requestMethods = {
			...
			convertToOpenid   :'user/convert_to_openid'	
		}
	}
	/**
	 * [getOpenid userid与openid互换]
	 * @param  {[Object]} convertParams 
	 * {
		   "userid": "zhangsan",
		   "agentid": 1
		}
	 ...
	 */
	async getOpenid(convertParams) {
		return await this.handlePostRequest('convertToOpenid',convertParams);
	}
	/**
	...
	/**
	 * [inviteUser 邀请用户加入企业微信]
	 * @param  {[type]} invite [邀请条件]
	 * {
	...
	async inviteUser(invite) {
		return await this.handlePostRequest('inviteUser',invite);
	}
	/**
