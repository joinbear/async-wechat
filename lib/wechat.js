...
class Wechat {
	 ...
	 * @param  {Boolean} isAgent  [是否是应用签名]
	 * @return {Object}           [签名对象]
	 */
	async getJsApiSignature(appName, url, isAgent = false) {
		const { mpPrefix, saveToken, getToken ,getAccessToken } = this;
		const cacheName = isAgent ? `${appName}_agent_ticket` : `${appName}_ticket`;
		const cacheTicket = await getToken(cacheName);
		if(cacheTicket && typeof cacheTicket !== undefined && cacheTicket !== 'undefined'){
			return Util.ticket(cacheTicket, url);
		}
		const accessToken = await getAccessToken(appName);
		const fetchUri = isAgent ? `${mpPrefix}ticket/get?access_token=${accessToken}&type=agent_config` : `${mpPrefix}get_jsapi_ticket?access_token=${accessToken}`;
		try {
			const jsapiTicket = await Util.handleRequest({ requestUrl: fetchUri });
			await saveToken(cacheName,jsapiTicket['ticket'],jsapiTicket['expires_in']);
			return Util.ticket(jsapiTicket['ticket'],url);
		}catch(error) {
			throw error;
		}
	}
	async getopenapprovaldata(appName, thirdNo) {
		const { mpPrefix, getAccessToken } = this;
	
		const accessToken = await getAccessToken(appName);
		const fetchUri = `${mpPrefix}corp/getopenapprovaldata?access_token=${accessToken}`;
		try {
			const data = await Util.handleRequest({ type: 'post', requestUrl: fetchUri, postData: {
				thirdNo: thirdNo,
			} });
			return data;
		} catch (error) {
