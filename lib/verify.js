...
class Verify {
	...
	verify(ctx) {
		...
		switch(this.type){
			case 'work':
				// 企业微信默认加密模式,echostr参数存在为get验证，post则无echostr参数加密密文在xml中
				console.log(signature);
				console.log(this.cryptor.getMsgSignature(timestamp, nonce, echostr));
				return signature !== this.cryptor.getMsgSignature(timestamp, nonce, echostr);
			break;
			case 'wx':
				// 加密模式
	     	const encrypted = !!(query.encrypt_type && query.encrypt_type === 'aes' && query.msg_signature);
	...
	recieveMessage(ctx){
		...
		return new Promise((resolve,reject)=>{
			ctx.req.on('data',(trunk)=>{
		  	...
		  	ctx.req.on('end',()=> {
		  		try {
		  			const decryptMessage = Template.decryptMessage(this.cryptor,postData);
		  			ctx.query.echostr = decryptMessage.encrypt;
		  			if(verify(ctx)){
							ctx.status = 401;
							ctx.body = 'Invalid signature';
							reject('Invalid signature');
						}
		      	ctx.weixin = decryptMessage.message;
		      	resolve(decryptMessage.message);
		  		}catch(error){
		  			console.log(error);
		  			reject(error);
		  		}
